...
Note: `emerg-shutdown.service` is disabled by default in the systemd preset
(`/usr/lib/systemd/system-preset/50-security-misc.preset`). It must be enabled
manually by running `systemctl enable emerg-shutdown.service` as root.
...
- `SSH client`: Hardens the SSH client by restricting key exchange to
  ...
  authentication to Ed25519 only, restricting ciphers to authenticated
  encryption modes, and enforcing encrypt-then-MAC.
...
- A `virusforget` script detects unauthorized changes to shell startup files and
  XDG autostart directories by comparing them against a stored baseline. Work in
  progress - the implementation will likely change significantly.
...
- Block unsafe passwordless logins for privileged accounts over insecure PAM
  services. Enforces account/session separation between `sysmaint` and regular
  user accounts.
...
Comprehensive USB device authorization rules are shipped for USBGuard.
All new USB devices are blocked by default (`ImplicitPolicyTarget=block`),
while devices connected at boot time are allowed.
...
- Reject devices with RNDIS interfaces due to known protocol-level
  buffer overflow vulnerabilities.
- Allow only one keyboard and one mouse at a time.
- Allow USB audio, video, mass storage, and hub devices.
- IPC access is restricted to root and members of the `plugdev` group.
...
- `emerg-shutdown.service` - Emergency shutdown on boot media removal / panic key.
